        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                if (json['success']) {
                    ...
                    setTimeout(function () {
                        $('#cart > button').html('<img src="/image/unilight/icons/cart.svg" alt=""><span class="caption">Корзина' + '<span id="container-for-total-count" class="item-count">' + json['total'] + '</span>' + '<span id="container-for-total-price" class="item-price">' + json['text_currency'] + '</span>' + '</span>');
                        $('#cart').children().find('[data-id="container-for-total-count"]').html(json['total']);
                        $('#cart').children().find('[data-id="container-for-total-price"]').html(json['text_currency']);
                    }, 100);

                    /*
                     setInterval(function () {
                         $('#cart').children().find('#container-for-total-count').html($('#cart').children().find('#cart-total-count').text());
                         $('#cart').children().find('#container-for-total-price').html($('#cart').children().find('#cart-total-price').text());
                     },500);*/
                    $('html, body').animate({ scrollTop: 0 }, 'slow');

                    //$('#cart > ul').load('index.php?route=common/cart/info ul li');
                    //quick-orde
                    //   console.log(' cart id'+$('#cart').children().find('.order-result').attr('id'));
