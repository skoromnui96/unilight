...
class ControllerNewsBlogArticle extends Controller {
	...
	public function index() {
		...
		if ($article_info) {
			...
			$data['text_another'] = $this->language->get('text_another');
			...
			foreach ($results as $result) {
				$data['images'][] = array(
                    ...
                    'thumb'    => $this->model_tool_image->resize($result['image'], 768, 461),
				);
			}
            ...
            if ($settings && isset($settings['show_comments_vk_id']) && isset($settings['show_comments_vk_article'])) {
            ...
            if ($settings && isset($settings['show_comments_fb_article']))
            $data['comments_fb'] = true;
            else
            $data['comments_fb'] = false;

            if ($settings && isset($settings['show_comments_dq_id']) && isset($settings['show_comments_dq_article']))
