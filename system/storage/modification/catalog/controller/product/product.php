...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_stock_none'] = $this->language->get('text_stock_none');
			...
			$data['text_lamps'] = $this->language->get('text_lamps');
		...
		$data['video_type'] = $this->config->get('config_video_view_type');
            ...
            $data['upc_description'] = $this->language->get('upc_description');
			...
			if ($product_info['video']) {
				if (preg_match("/http/i", $product_info['video'])) {
					print_r('<pre>Некорректные данные о видео в базе данных</pre>');
				} else {
					if (preg_match("/#separator#/i", $product_info['video'])) {
						$video = explode("#separator#", $product_info['video']);
						if (isset($video[0])) {
							$video_id = $video[0];
							$video_title = $video[1];
						}
		...
		if ($video_id) {
			if (!is_file(DIR_IMAGE . 'catalog/video/' .$video_id. '.jpg')) {
				$ch = curl_init('http://img.youtube.com/vi/' .$data['video']. '/mqdefault.jpg');
				$fp = fopen(DIR_IMAGE . 'catalog/video/' .$data['video']. '.jpg', 'wb');
			...
			if (version_compare(VERSION, '2.2', '>=')) {
				$image_additional_width = $this->config->get($this->config->get('config_theme') . '_image_additional_width');
				$image_additional_height = $this->config->get($this->config->get('config_theme') . '_image_additional_height');
			} else { 
			...
			$data['thumb_video'] =  $this->model_tool_image->resize('catalog/video/' .$video_id. '.jpg', $image_additional_width, $image_additional_height);
			$data['tab_thumb_video'] =  $this->model_tool_image->resize('catalog/video/' .$video_id. '.jpg', 320, 180);
			$data['default_video_title'] = $video_title;
		} else {
			$data['thumb_video'] = '';
			$data['tab_thumb_video'] = '';
			$data['default_video_title'] = '';
		}
			...
			foreach ($videos as $video) {
				$data['videos'][] = array(
					'id' => $video['video'],
					'title' => $video['title'],
					'thumb' => $this->model_tool_image->resize('catalog/video/' .$video['video']. '.jpg', 320, 180) 
				);
			}

	if ($product_info['video'] || $videos) {
	if (version_compare(VERSION, '2.2', '>=')) {
		...
		if (file_exists(DIR_TEMPLATE . $current_theme . '/stylesheet/tabvideo.css')) {
			$this->document->addStyle('catalog/view/theme/'. $current_theme .'/stylesheet/tabvideo.css');
		} else {
			$this->document->addStyle('catalog/view/theme/default/stylesheet/tabvideo.css');
		}
	} else {
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/stylesheet/tabvideo.css')) {
			$this->document->addStyle('catalog/view/theme/'. $this->config->get('config_template') . '/stylesheet/tabvideo.css');
		} else {
		$this->document->addStyle('catalog/view/theme/default/stylesheet/tabvideo.css');
		}
	}
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$product_option_value_data[] = array(
							...
							'image'                   => $this->model_tool_image->resize($option_value['image'], 80, 110),
							'price'                   => $price,
