...
class ModelCatalogProduct extends Model {
    public function addProduct($data) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', video_in_product = '" . $this->db->escape($data['video_in_product']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
        ...
        if (isset($data['video'])) {
            ...
            if (!empty($data['video'])) {
                ...
                if (preg_match("/#separator#/i", $data['video'])) {
                    $datavideo = explode("#separator#", $data['video']);
                    if (isset($datavideo[0])) {
                        $video_id = $datavideo[0];
                    }
                } else {
                    $video_id = $data['video'];
                }

                if ($video_id && !is_file(DIR_IMAGE . 'catalog/video/' .$video_id. '.jpg')) {
                    $ch = curl_init('http://img.youtube.com/vi/' .$video_id. '/mqdefault.jpg');
                    $fp = fopen(DIR_IMAGE . 'catalog/video/' .$video_id. '.jpg', 'wb');
                ...
                if (empty($data['video_title'])) {
                    ...
                    $default_title = str_replace("\"","'", $meta['title']);
                } else {
                    $default_title = $data['video_title'];
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET video = '" . $this->db->escape(html_entity_decode($video_id . '#separator#' . $default_title, ENT_QUOTES, 'UTF-8')) . "' WHERE product_id = '" . (int)$product_id . "'");
            } else {
        ...
        if (isset($data['product_video'])) {
            foreach ($data['product_video'] as $product_video) {
                ...
                if ($product_video['video']) {
                    $ch = curl_init('http://img.youtube.com/vi/' .$product_video['video']. '/mqdefault.jpg');
                    $fp = fopen(DIR_IMAGE . 'catalog/video/' .$product_video['video']. '.jpg', 'wb');
                    curl_setopt($ch, CURLOPT_FILE, $fp);
                    curl_setopt($ch, CURLOPT_HEADER, 0);
                    curl_exec($ch);
                    curl_close($ch);
                    ...
                    if (!$product_video['title']) {
                        $url = 'https://www.youtube.com/watch?v=' .$product_video['video'];
                        $meta = get_meta_tags($url);
                    ...
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_video SET product_id = '" . (int)$product_id . "', video = '" . $this->db->escape($product_video['video']) . "', title = '" . $this->db->escape($title) . "', sort_order = '" . (int)$product_video['sort_order'] . "'");
                }
            }
        }
    ...
    public function editProduct($product_id, $data) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', video_in_product = '" . $this->db->escape($data['video_in_product']) .  "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
